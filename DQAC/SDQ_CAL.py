...
# Paper: https://papers.nips.cc/paper/2010/file/091d584fced301b442654dd8c23b3fc9-Paper.pdf
# And implementation of Conservative Advantage Learning
...
class SDQ_CAL(object):
	...
	def train(self, replay_buffer, batch_size=256):
		...
		with torch.no_grad():
			# add advantage value into reward
			noise = (torch.randn_like(action) * self.policy_noise).clamp(min=-self.noise_clip, max=self.noise_clip)
			action_1 = (self.actor_1(state) + noise).clamp(min=-self.max_action, max=self.max_action)
			action_2 = (self.actor_2(state) + noise).clamp(min=-self.max_action, max=self.max_action)
			if not self.pure_al:
				Q_mid = torch.min(self.critic_target_1(state, action), self.critic_target_2(state, action))
				reward_Q1 = self.beta * (Q_mid - self.critic_target_1(state, action_1))
				reward_Q2 = self.beta * (Q_mid - self.critic_target_2(state, action_2))
			else:
				reward_Q1 = self.beta * (self.critic_target_1(state, action) - self.critic_target_1(state, action_1))
				reward_Q2 = self.beta * (self.critic_target_2(state, action) - self.critic_target_2(state, action_2))

			# Select action according to policy and add clipped noise
			next_noise = (torch.randn_like(action) * self.policy_noise).clamp(min=-self.noise_clip, max=self.noise_clip)
			next_action_1 = (self.actor_1(next_state) + next_noise).clamp(min=-self.max_action, max=self.max_action)
			next_action_2 = (self.actor_2(next_state) + next_noise).clamp(min=-self.max_action, max=self.max_action)

			# Compute the target Q value
			target_Q1 = self.critic_target_2(next_state, next_action_1)
			target_Q2 = self.critic_target_1(next_state, next_action_2)
			target_Q1 = reward + reward_Q1 + not_done * self.discount * target_Q1
			target_Q2 = reward + reward_Q2 + not_done * self.discount * target_Q2
